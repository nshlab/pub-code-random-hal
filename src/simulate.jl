...
function binary_scm(d, d_first, ρ = 0.1)
    ...
    dgp = @dgp(
       ...
       p = 10 .* sqrt(d_first) .* (vec(mean(L[:,1:d_first] .- L[:,1:d_first] .^ (1/2), dims = 2))),
       A ~ Bernoulli.(logistic.(p .+ 1.5 .* sqrt(d_first))),
       μ = A .* (sin.(1.15*pi * C).^3 .+ C .^ 2) .- p,
       Y ~ Normal.(μ, 0.5)
    )
