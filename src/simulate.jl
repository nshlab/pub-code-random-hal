...
#        p = sqrt(d_first) .* (vec(mean(sin.(0.5 .* pi .* sin.(0.5 .* pi .* L[:,1:d_first])), dims = 2))),
#        A ~ Bernoulli.(logistic.(p / sqrt(d_first))),
#        μ = 0.5 .* A .* (sin.(0.7 .* pi .* C).^3) .- (1.5 .* C) .+ p,
#        Y ~ Normal.(μ, 1.0)
#     )
...
function binary_scm(d, d_first, ρ = 0.05)
    ...
    dgp = @dgp(
       ...
       L ~ SklarDist(GaussianCopula(d, ρ), Tuple(fill(Uniform(-1,1), d))),
       p = sqrt(d_first) .* (vec(mean(sin.(0.5 .* pi .* L[:,1:d_first]), dims = 2))),
       A ~ Bernoulli.(logistic.(p)),
       μ = 0.5 .* A .* (sin.(0.7 .* pi .* C).^3) .+ p,
       Y ~ Normal.(μ, 1.0)
    )
